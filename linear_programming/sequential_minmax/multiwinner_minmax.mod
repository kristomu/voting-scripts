...
# This is the "minimize strongest defeat" formulation, consistent with 
# Wikipedia's article on minmax.
...
s.t. max_opposition_bound{i in CANDIDATES}:
	if i != cand_to_eval then
		max_opposition >= sum {b in BALLOTS} (
